...
git stash && git checkout master && git fetch && git reset --hard origin/master && git pull --rebase origin master
for dir in ./*; do (cd "$dir" && git stash && git checkout master && git fetch && git reset --hard origin/master && git pull --rebase origin master); done
cd /var/www/invision
sudo rm -rf /var/bak
sudo mkdir /var/bak/
